...
\begin{equation}\label{2ddengjia}
    \left \{
        \begin{aligned}
            ...
            &u = u_g, \; v = v_g, \quad &&x = x_g \;  \\
            &EA u_{,x}  = P, \; u_{,x} = v_{,x}, \quad &&x = x_t \;  \\
            &u= u_0, \quad \dot{u} = v_0, && t=t_0 \;  \\
            &\dot{u} = \dot{v} && t=t_1 
        \end{aligned}
    \right .
\end{equation}
...
+ \frac{\beta}{2}\int_{x_0}^{x_1} \left(v \vert_{t=t_1}-v_1\right)^2 dx
+ \frac{\beta}{2}\int_{t_0}^{t_1} \left(v\vert_{x=x_g}-v_g\right)^2 dx
\end{equation}
...
\begin{equation}
    \begin{aligned}
        ...
        &+ \beta\int_{x_0}^{x_1} \left(v \vert_{t=t_1}-v_1\right) \delta v \vert_{t=t_1} dx
        + \beta\int_{t_0}^{t_1} \left(v\vert_{x=x_g}-v_g\right) \delta v \vert_{x=x_g} dt
    \end{aligned}
\end{equation}
...
\begin{equation}\label{3.29}
    \left \{
        \begin{aligned}
            ...
            f^\alpha_{I}  &= \int_{x_0}^{x_1} \alpha N_I u \vert_{t=t_0} dx + \int_{t_0}^{t_1} \alpha N_I u\vert_{x=x_0} dt
        \end{aligned}
    \right .
\end{equation}
\begin{equation}\label{3.30}
    \left \{
        \begin{aligned}
            ...
            f^\beta_I &= \int_{x_0}^{x_1} \beta N_I  v \vert_{t=t_1} dx + \int_{t_0}^{t_1} \beta N_I v \vert_{x=x_g} dt
        \end{aligned}
    \right .
\end{equation}
...
有效替换并且避开了传统弱形式数值求解体系中无法在“将来”时间极点施加末端位移$\delta u(t_1)=0$约束的困难。
我们顺利构造出一个关于真实求解位移量$u$以及共轭物量$v$的完全对称弱形式变分问题，
...
\begin{figure}[!h]
    \centering
    \includegraphics[scale=0.8]{fig/一维弦横向振动.png}
...
\begin{figure}[H]
    \centering
        \begin{tabular}{c@{\hspace{20pt}}c}
            ...
            $c=0.03$网格示意图 & $c=0.04$网格示意图  \\[10pt]
...
从上图可以看出，采用非结构化网格结合Delaunay三角形局部细化方案计算时仍然会出现振荡的现象，
同时也得不到理论的误差收敛率。因此，我们采用结构化三角形网格结合最长边二分法进行局部细化方案来计算，
该方案生成的网格演化图如下：
