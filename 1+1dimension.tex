...
\end{equation}
其中，$a(u,u)$为双线性形式，$f(u)$为线性形式，表达式如下：
\begin{equation}
    a(u,u) = \int_{t_0}^{t_1} \int_{x_0}^{x_1} (\dot u \rho A \dot u - u_{,x} EA u_{,x}) dx dt
\end{equation}
\begin{equation}
    f(u) = \int_{t_0}^{t_1} (\int_{x_0}^{x_1} ub dx  + uP \vert_{x=x_t})dt
\end{equation}
...
可以找到一个精确解$u \in \mathcal U$使得该问题的能量泛函变分等于$0$：
...
其中，$\boldsymbol{u}_n$、$\dot{\boldsymbol{u}}_n$、$\ddot{\boldsymbol{u}}_n$分别为$t_n$时刻的位移、速度和加速度向量，
...
首先，由于试函数空间$\mathcal{U}$和权函数空间$\mathcal{P}$不一致将会导致刚度矩阵不为方阵，无法求解。
所以我们通过将传统能量泛函中的虚位移$\delta u$作为拉格朗日乘子，
换成一个拉格朗日乘数$v$的表达形式来代替原来无法数值实现的虚位移$\delta u$，
再将势函数空间投影到$\mathcal{V}$中，设计全新拉格朗日乘子型能量泛函。
再引入以下双线性形式和线性形式来表达拉格朗日变分问题：
\begin{equation}\label{3.16}
\bar \Phi(u,v) = \Phi(u) - a(u,v) + f(v)
\end{equation}

通过引入与变量$u$所属空间不同的虚拟变量$v$来进行变分推导，
可以将其看作将变量$u$投影到虚拟空间来求$\bar \Phi$的极小值。
然后对总能量泛函变分得到对应的波动问题的弱形式：
\begin{equation}\label{3.24}
    \begin{aligned}
        \delta \bar \Phi(u,v) = \delta \Phi(u) &- \int_{t_0}^{t_1} \int_{x_0}^{x_1} (\delta \dot u \rho A \dot v - \delta u_{,x} EA v_{,x}) dx dt - \int_{t_0}^{t_1} \int_{x_0}^{x_1} (\delta \dot v \rho A \dot u - \delta v_{,x} EA u_{,x}) dx dt \\
        &+ \int_{t_0}^{t_1} \left( \int_{x_0}^{x_1} \delta v b dx + \delta v P \vert_{x=x_t} \right) dt = 0
    \end{aligned}
\end{equation}

引入相同节点的有限元近似同时离散$u$和$v$，对上式进行整理可得：
找到一组解$(u_h,v_h) \in (\mathcal{U}_h \times \mathcal {v}_h)$，使得
\begin{equation}\label{3.21}
    \text{Find } u \in \mathcal U, v \in \mathcal V, \quad
    \left \{
    \begin{aligned}
        a(\delta u, u) - a(\delta u, v) &= 0, \quad & \forall \delta u \in \mathcal U \\
        -a(u, \delta v) &= -f(\delta v), \quad & \forall \delta v \in \mathcal V
    \end{aligned}
    \right .
\end{equation}

将动能表达式\eqref{T_2d}和势能表达式\eqref{U_2d}代入可得：
\begin{equation}\label{3.26}
    \left\{
    \begin{aligned}
        &\int_{t_0}^{t_1} \int_{x_0}^{x_1} \left( \delta\dot{u}\rho A\dot{u} - \delta u_{,x}EA u_{,x} \right) dx dt - \int_{t_0}^{t_1} \int_{x_0}^{x_1} \left( \delta\dot{u}\rho A\dot{v} - \delta u_{,x}EA v_{,x} \right) dx dt = 0 \\
        &-\int_{t_0}^{t_1} \int_{x_0}^{x_1} \left( \delta\dot{v}\rho A\dot{u} - \delta v_{,x}EA u_{,x} \right) dx dt = -\int_{t_0}^{t_1} \left( \int_{x_0}^{x_1} \delta v b dx + \delta v P \vert_{x=x_t} \right) dt
    \end{aligned}
    \right.
\end{equation}

采用传统有限元法进行分析时，对位移变量$u$和拉格朗日乘子$v$进行有限元近似，其近似值$u^h$和$v^h$具有如下表达式：
\begin{equation}\label{uv_h}
u^h (x,t)=v^h (x,t) = \sum_{I=1}^{n_p} N_I (x,t) d_I
\end{equation}
对时间的一阶导数的近似值$\dot u^h$和$\dot v^h$为：
\begin{equation}\label{dotuv_h}
    \dot u^h =\dot v^h= \sum_{I=1}^{n_p} N_{I,t} d_I
\end{equation}
对位移的一阶导数的近似值$u_{,x}^h$和$v_{,x}^h$为：
\begin{equation}\label{uv_x^h}
    u_{,x}^h=v_{,x}^h= \sum_{I=1}^{n_p} N_{I,x} d_I
\end{equation}
...
通过有限元近似表达式对真实位移变量 $u$ 及其对应的虚拟动量对应变量 $v$（即拉格朗日乘子）进行相同时空边界节点的初步离散（即 $u_h = \sum N_I u_I$, $v_h = \sum N_I v_I$），由于式\eqref{3.26}中所提时空混合离散伽辽金弱形式中双线性算子均为同一个时空算子，我们可以将未考虑本质边界条件的最简离散控制方程表示为：
...
其中，时空总刚度矩阵 $\boldsymbol{K}$ 重复在三个分块位置处被使用，因此在有限元数值求解阶段，完全可以利用该局部对称且同质的特点来优化程序内核结构，极大降低系统维度的内存开销。

从上式可以看出，试函数空间和权函数空间均属于相同的离散空间，$\mathcal{U}_h\times \mathcal{V}_h$。
在施加强制边界条件时，将不再出现由于空间不匹配造成施加困难的问题。本文采用罚函数法对$u$和$v$相关的本质边界条件进行施加。
罚函数的增广能量泛函可写为：
\begin{equation}
\tilde{\Phi}(u,v)
= \bar \Phi(u,v)
+ \frac{\alpha}{2}\int_{x_0}^{x_1} \left(u \vert_{t=t_0}-u_0\right)^2 dx
+ \frac{\beta}{2}\int_{x_0}^{x_1} \left(v\vert_{t=t_1}-\bar v_1\right)^2 dx
\end{equation}
...
\begin{align}
K^\alpha_{IJ} &= \int_{x_0}^{x_1} \alpha N_I N_J \vert_{t=t_0} dx, \quad 
f^\alpha_I = \int_{x_0}^{x_1} \alpha N_I u_0 \vert_{t=t_0} dx \label{3.29} \\
...
这样，我们通过引入一个在拉格朗日维度投影等价的独立虚变量 $v$，有效替换并且避开了传统弱形式数值求解体系中无法在“将来”时间极点施加末端位移 $\delta u(t_1)=0$ 约束的痛点。我们顺利构造出一个关于真实求解物量 $u$ 以及共轭物量 $v$ 的完全对称弱形式变分问题。在经过罚函数结合统一的单元形函数向有限元离散化后形成的系数矩阵及力列阵性质优良，直接应用代数求解器便得以对动力学的初值问题彻底完成一站式的空间与时间的全维度连续推进。该框架为基于哈密顿变分推导出的“1+1”维复杂弹性阻抗以及边界不连续混合网格动力波动问题，奠定了高度统一且高并行的离散数值计算基石。
...
在解决时空有限元法在时间末端边界条件的问题时，根据最小势能原理，
我们把原能量泛函变分得到的弱形式当作一个拉格朗日乘子项加入到能量泛函中进行变分。
为与上述一维波动问题相对应，虚位移$\delta u$用独立的虚拟变量$v$表示，
可以得到一维双变量的能量泛函变分表达式等于$0$：
\begin{equation}
    \begin{aligned}
        \delta\Phi(u, v) &= \int_{t_0}^{t_1}\int_{x_0}^{x_1} (\delta\dot{u}\rho A\dot{u} - \delta u_{,x}EA u_{,x}) dx dt \\
        &\quad - \int_{t_0}^{t_1}\int_{x_0}^{x_1} (\delta\dot{v}\rho A\dot{u} - \delta v_{,x}EA u_{,x}) dx dt \\
        &\quad - \int_{t_0}^{t_1}\int_{x_0}^{x_1} (\delta\dot{u}\rho A\dot{v} - \delta u_{,x}EA v_{,x}) dx dt \\ 
        &\quad - \int_{t_0}^{t_1} \left( \int_{x_0}^{x_1} \delta v b dx + \delta v P \vert_{x=x_t} \right) dt= 0
    \end{aligned}
\end{equation}

对上述弱形式方程的时间项和空间项分别做分部积分，可得：
\begin{equation}
    \begin{aligned}
        ...
        & + \int_{t_0}^{t_1}\int_{x_0}^{x_1} \delta v (\rho A \ddot{u} - EA u_{,xx} - b) dx dt - \int_{x_0}^{x_1} \delta v \rho A \dot{u} \big|_{t_0}^{t_1} dx \\
        & \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad + \int_{t_0}^{t_1} \delta v \left( EA u_{,x} n_x \big|_{x_0}^{x_1} - P \vert_{x=x_t} \right) dt \\
        & + \int_{t_0}^{t_1}\int_{x_0}^{x_1} \delta u (\rho A \ddot{v} - EA v_{,xx}) dx dt 
        - \int_{x_0}^{x_1} \delta u \rho A \dot{v} \big|_{t_0}^{t_1} dx \\
        & \quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad\quad + \int_{t_0}^{t_1} \delta u EA v_{,x} n_x \big|_{x_0}^{x_1} dt = 0
    \end{aligned}
\end{equation}
...
其中，根据变分法基本引理，由于任意虚位移 $\delta u$ 和 $\delta v$ 在时空求解域内部 $(x_0, x_1) \times (t_0, t_1)$ 可以任意取值，必须使得其双重积分项的系数为零。由内部的 $\delta v$ 积分项立刻给出波动方程的基本控制形式：
\begin{equation}
    \rho A \ddot{u} = EA u_{,xx} + b
\end{equation}
将其控制方程代入 $\delta u$ 的内部积分系数项中，不难得到虚拟变量 $v$ 在求解域内同样需满足相同的动力学平衡方程：
\begin{equation}
...
\end{equation}
同时，由于在强制边界条件处（例如两端端部固定，区间 $[x_0,x_1]$ 对应的 $x=x_g$），位移给定，因此对应的虚位移 $\delta u$ 及虚拟变量变分 $\delta v$ 在该强制边界上也必须为零；或者在自然力边界 $\Gamma_t$（如受外力 $P$）处应满足力的动量平衡调节，由此可以得到空间边界条件为：
\begin{equation}
...
如式\eqref{3.34}所示，基于拉格朗日变分方案的真实位移变量 $u$ 完美满足经典动力学波方程的强形式控制方程。
同时，引入的虚拟变量 $v$ 不仅在时空内部展现出与真实位移相互匹配的一致动力学行为，
而且在自然边界以及时间的末端边界也确保了连续协调。综上所述，
我们在时空混合网格求解框架下，完整推导并证明了该能量泛函弱形式与经典波动强形式之间的等价关系。
...
考虑一维弹性杆撞击刚性墙的情况，如图\ref{fg:impact_bar}所示，弹性杆的材料参数均取为单位值，即弹性模量 $E=1$，材料密度 $\rho=1$，杆总长 $L=1$。
在初始时刻，弹性杆整体具有向着刚性墙撞击的均匀初始速度 $v_0=1$。在撞击发生时，杆的左端受到刚性墙的阻挡，因此在该处施加强制的零位移边界条件：在任意时刻均始终满足 $u(x=0,t)=0$。同时，杆的右端不受外力约束，作为自由端施加自然边界条件。
由于杆的一端固定，另一端以均匀初速度运动，因此在这个一维波动问题中，杆内将会产生一个不断传播的压缩应力波，应力的解析表达式为 $\sigma = EAu_{,x}$。
...
为了对上述计算域进行数值求解，
本文分别采用不同疏密度的三节点（Tri3）和六节点（Tri6）的三角形单元均布网格进行离散。
图\ref{fg:pengzhuangTri3}和图\ref{fg:pengzhuangTri6}分别展示了一维弹性杆撞击刚性墙问题中，
在时空域上采用不同网格的节点模型及其对应的应力云图。
...
\begin{figure}[H]
    \centering
        \begin{tabular}{cccc}
            ...
            \includegraphics[width=0.2\textwidth]{fig/Tri3_32.png}\makebox[0pt][l]{\hspace{5pt}\includegraphics[width=0.1\textwidth]{fig/legend.png}} \\
            $4\times8$ & $8\times16$ & $16\times32$ &  $32\times64$ \\
        \end{tabular}
        \caption{Tri3均布网格弹性杆问题不同密度下的节点模型以及应力云图}
...
\begin{figure}[H]
    \centering
        \begin{tabular}{cccc}
            ...
            \includegraphics[width=0.2\textwidth]{fig/Tri6_32.png}\makebox[0pt][l]{\hspace{5pt}\includegraphics[width=0.1\textwidth]{fig/legend.png}} \\
            $4\times8$ & $8\times16$ & $16\times32$ &  $32\times64$ \\
        \end{tabular}
...
当我们采用六节点三角形单元网格\ref{fg:yiweixianliujiedianwangge}进行离散时该问题时，
图\ref{fg:yiweixianliujiedianweiyitu}为该节点离散情况下的位移计算结果，都具有很好的稳定性。
它的位移误差和能量误差也均能达到理论收敛率，如图\ref{fg:yiweixianliujiedianL2}所示。
\begin{figure}[!h]
    \centering
        \begin{tabular}{c@{\hspace{20pt}}c}
            ...
            $17\times17$ & $33\times33$ \\
        \end{tabular}
        \caption{一维弦自由横向振动问题三角形六节点单元模型}\label{fg:yiweixianliujiedianwangge}
\end{figure}
