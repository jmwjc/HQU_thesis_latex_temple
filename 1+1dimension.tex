...
该材料的材料密度为$\rho$，材料的弹性模量为$E$，材料受沿$x$方向的体力$b$和外力$P$作用。
左端为强制边界条件$x_g$，右端为自然边界条件$x_t$。
...
\end{equation}
式中，$A$为材料横截面积，$u$和$\dot u$为广义自由度和其对时间$t$的一阶导数，
$u,x$为广义自由度对位移$x$的一阶导数。
...
\end{equation}
其中，$\boldsymbol M$为质量矩阵，$\boldsymbol K$为刚度矩阵，$\boldsymbol f_{n+1}$为力向量，
$\boldsymbol u_{n+1}$为结点位移,$\ddot {\boldsymbol u}_{n+1}$为结点位移对时间的二阶导数。
它们的分量表达式如下：
\begin{equation}
    \left \{ 
        \begin{aligned}
...
Newmark-$\beta$法便是典型代表，它在时间域采用基于强形式的差分近似，
在空间域通过变分原理建立弱形式，形成耦合迭代格式。

Newmark-$\beta$法对位移、速度和加速度的时间导数采用如下近似：
...
最后联立时间离散格式与空间离散方程式\eqref{Mu+Ku=f}，消去速度项后得到Newmark-$\beta$法的迭代公式：
...
\begin{equation}\label{3.24}
    \begin{aligned}
        \delta \bar \Phi(u,v) = \delta \Phi(u) &- \int_{t_0}^{t_1} \int_{x_0}^{x_1} (\delta \dot u \rho A \dot v - \delta u_{,x} EA v_{,x}) dx dt \\
...
$\text{Find } u_h \in \mathcal{U}_h, v_h \in \mathcal{V}_h$，使得
...
\begin{equation}\label{3.26}
    \left\{
    \begin{aligned}
        &\int_{t_0}^{t_1} \int_{x_0}^{x_1} \left( \delta\dot{u}\rho A\dot{u} - \delta u_{,x}EA u_{,x} \right) dx dt \\
        &\quad - \int_{t_0}^{t_1} \int_{x_0}^{x_1} \left( \delta\dot{u}\rho A\dot{v} - \delta u_{,x}EA v_{,x} \right) dx dt = 0 \\
        &-\int_{t_0}^{t_1} \int_{x_0}^{x_1} \left( \delta\dot{v}\rho A\dot{u} - \delta v_{,x}EA u_{,x} \right) dx dt \\
        &\quad = -\int_{t_0}^{t_1} \left( \int_{x_0}^{x_1} \delta v b dx + \delta v P \vert_{x=x_t} \right) dt
    \end{aligned}
    \right.
\end{equation}
...
\begin{equation}
    \begin{aligned}
        ...
        + \int_{t_0}^{t_1} \delta v \left( EA u_{,x} n_x \big|_{x_0}^{x_1} - P \vert_{x=x_t} \right) dt \\
...
我们在时空混合网格求解框架下，完整推导并证明了该能量泛函弱形式与经典波动问题强形式之间的等价关系。
