...
FIXME 左端为强制边界条件$x_g$，右端为自然边界条件$x_t$。

\begin{figure}[!h] 
    \centering
    \includegraphics[scale=0.9]{fig/一维波动问题示意图.png}
\caption{\centering{一维弹性杆问题示意图,
FIXME:图太差了，重新画，不清晰，$x_0$和$x_1$画在$x$轴上。$x_g$是啥？你写在那边人家看得懂吗？$E, \rho$又是啥？不要画在图上，你又没办法表示，力的颜色用不同的颜色进行区分。}}
...
\end{equation}
为了后续表述方便，这里引入双线性算子表示哈密顿量为：
\begin{equation}
    \Phi(u) = \frac{1}{2} a(u,u) - f(u)
\end{equation}
其中，$a(\bullet,\bullet):\mathcal U \times \mathcal U \rightarrow \mathcal R$为双线性算子，$f(\bullet):\mathcal U \rightarrow \mathcal R$为线性算子，其表达式如下所示：
\begin{equation}
...
\end{equation}
同时，由于$\delta u$在位移边界条件处$x=x_g$也必须为零，因此可以得到：
\begin{equation}
...
即在时间维度采用分部积分而空间部分不变。
此时，式\eqref{eq:1d weak found}可改写为：
FIXME:
\begin{multline}
...
\begin{multline}
    ...
    \int_{t_0}^{t_1} \delta u P\vert_{x_t} dt + \int_{t_0}^{t_1} \int_{x_0}^{x_1} \delta u b dx dt
\end{multline}
对上式采用配点法进行时间维度离散，要求在给定的任意一$t = t_{n+1}$时刻满足：
...
式\eqref{eq:1d_iter_weak_form}中的时间强形式通常以差分为基础的时间离散进行求解，如Newmark-$\beta$法便是典型代表，该方法采用如下差分格式对位移、速度和加速度近似为：
...
FIXME:优缺点，放在第一次出现的地方，加讨论，如算例几所示。
...
FIXME:为了建立完全弱形式下的时空混合离散（时间维度和空间维度均不做分部积分），必须得施加虚位移边界条件\eqref{}。
本小节将第\ref{}章所提……，引入\eqref{}，从而构造全新路径施加虚伪本质边界条件。

FIXME
首先，在有限元离散的完全弱形式\eqref{eq:1d weak form}试函数近似空间$\mathcal{U}_h$和权函数近似空间$\mathcal{V}_h$并不相同，权函数近似空间$\mathcal V_h$要求其任意元素在时间末端处为零，即$\delta u(x,t_1)=0$。
% \begin{equation}
%     \mathcal V_h = \{\delta u_h \in \mathcal U_h \vert \delta u_h(t_1) = 0\}
% \end{equation}
将传统能量泛函\eqref{}中的虚位移$\delta u$作为拉格朗日乘子，并将其采用$v$表示。
进一步，将该拉格朗日乘子项引入式\eqref{}中的哈密顿量，可得如下全新能量泛函表达式：
...
根据$\delta u$和$\delta v$的任意性，同时引入相同的有限元离散近似$u$和$v$，即$u_h$和$v_h$，可得如下伽辽金弱形式：
FIXME
