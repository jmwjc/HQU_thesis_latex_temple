...
\end{equation}
式中，$T$为动能，$U$为势能。
根据哈密顿原理\cite{arnold1978}，该问题的精确解$q$使得式\eqref{eq:functional}的变分等于$0$，即：找到一个$q\in Q$，使得
...
\end{equation}
其中，$Q$为试函数空间满足$q\in Q$，$q_0$和$\dot q_0$为$q$在$t_0$时刻的初始位移和速度。
而$P$为权函数空间，满足$\delta q\in Q'$。
...
所以可以称式\eqref{eq:euler-lagrange}为强形式方程(\textbf{Strong Found})，
而式\eqref{eq:variation}为弱形式方程(\textbf{Weak Found})。
...
通过当前时刻(第n步)的状态(位移$u_n$、速度$v_n$​、加速度$a_n$)，
计算下一时刻(第n+1步)的状态，迭代公式中$\Delta t$为时间步长，
$m$为质量，$c$为阻尼系数，$k$为刚度，$f_{n+1}$为第$n+1$步外力。
当取$\gamma=0$、$\beta=0$时，Newmark-$\beta$法为显式格式，
采用向前差分近似时间导数，仅利用当前时刻(第n步)的状态计算下一时刻(第n+1步)状态，
无需求解方程组，迭代公式为：
\begin{equation}\label{newmark-explicit}
    \left \{
        \begin{split}
            a_n &= \frac{1}{m}(f_n - c v_n - k u_n) \\
...
采用向后差分近似时间导数，利用下一时刻(第n+1步)的状态反推更新，
...
将动能和势能表达式\eqref{eq:kinetic-potential}代入到弱形式\eqref{eq:variation}，可得如下问题：找到一个$q_h\in Q_h$，使得
...
将有限元近似表达式\eqref{2.13}和式\eqref{dotq_h}代入弱形式\eqref{eq:variation}，
可得到离散控制方程为：
...
然而，如图\ref{fg:δq=0}所示，直接令$\delta q_h(t_1)=0$，将导致刚度矩阵$\boldsymbol K$不为方阵。
...
\begin{figure}[!h]
    \centering
    \includegraphics[scale=0.75]{fig/math/边界条件转移图.png}
...
然后是计算效率低，数值求解中针对对称矩阵的优化算法可将计算复杂度从$O(n^3)$降低至$O(n^2)$,
