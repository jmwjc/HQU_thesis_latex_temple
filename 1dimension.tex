...
根据拉格朗日动力学，
FIXME
有于系统的质量矩阵$\boldsymbol{M}$与外力向量$\mathbf{F}$在整个运动过程中均为常数，系统具有恒定的加速度$\mathbf{a}$：
\begin{equation}
\mathbf{a} = \mathbf{M}^{-1} \mathbf{F} = \begin{bmatrix} a_X \\ a_s \end{bmatrix}
\end{equation}
其中，$a_X$、$a_s$分别为楔形滑块和小滑块的加速度，
结合零初始速度和位移，任意时刻$t$的位移解析解符合标准匀加速运动学公式：
\begin{equation}
\mathbf{q}(t) = \frac{1}{2} \mathbf{a} t^2 \implies
\begin{cases}
X(t) = \frac{1}{2} a_X t^2 \\
s(t) = \frac{1}{2} a_s t^2
\end{cases}
\end{equation}
