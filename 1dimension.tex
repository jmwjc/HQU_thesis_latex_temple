...
\end{equation}
其中，$\mathcal Q_h$为有限元近似空间：
\begin{equation}
    \mathcal Q_h = \textrm{span} \{N_I\}_{I=1}^{n_p}
\end{equation}

值得注意的是，为了施加初始时刻动量边界条件，此时的近似权函数空间$\mathcal{P}_h$需要满足$\delta q_h(t_1)=0$，但不再要求$\delta q_h(t_0)=0$，即：
\begin{equation}
    \mathcal P_h = \{q_h \in \mathcal Q_h \vert \delta q_h(t_1) = 0\}
\end{equation}
