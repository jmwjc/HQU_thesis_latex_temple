...
而$\mathcal{P}$为权函数空间，满足$\delta q\in \mathcal{Q}'$。
...
将动能和势能表达式\eqref{eq:kinetic-potential}代入到弱形式\eqref{eq:variation}，可得如下问题：找到一个$q_h\in \mathcal{Q}_h$，使得
\begin{equation}\label{eq:discrete-variation}
    \int_{t_0}^{t_1} \left( \delta \dot q_h m \dot q_h - \delta q_h k q_h \right) dt = \left. \delta q_h m \dot q_h \right|_{t=t_0} \quad \forall \delta q_h \in \mathcal{P}_h
\end{equation}

值得注意的是，为了施加初始时刻动量边界条件，此时的近似权函数空间$\mathcal{P}_h$需要满足$\delta q_h(t_1)=0$，但不再要求$\delta q_h(t_0)=0$。
...
在使用伽辽金法进行求解的过程中，试函数空间$\mathcal{Q}_h$和权函数空间$\mathcal{P}_h$采用相同的节点数进行离散，即$\dim \mathcal{Q}_h = \dim \mathcal{P}_h$。
...
\end{multline}
引入相同节点的有限元近似同时离散$q$和$p$，整理可得：找到一组解$(q_h,p_h) \in \mathcal{Q}_h \times \mathcal{P}_h$，使得
\begin{equation}
    \begin{aligned}
        \int_{t_0}^{t_1} (\delta \dot q m \dot q - \delta q k q) dt - \int_{t_0}^{t_1} (\delta \dot q m \dot p - \delta q k p) dt &= 0, \quad & \forall \delta q_h \in \mathcal{Q}_h \\
        - \int_{t_0}^{t_1} (\delta \dot p m \dot q - \delta p k q) dt &= - \delta p m  \dot q \vert_{t=t_0}, \quad & \forall \delta p_h \in \mathcal{P}_h
    \end{aligned}
\end{equation}

为了验证引入拉格朗日乘子后的双变量弱形式与原有强形式控制方程之间的等价性，对上述弱形式方程的时间项分别做分部积分。对于关于虚位移$\delta q$的变分方程，进行分部积分可得：
\begin{equation}
    \begin{aligned}
...
结合边界初始项对应，整理化简为：
\begin{equation}
    \int_{t_0}^{t_1} \delta p (m\ddot q + k q ) dt - \delta p m \dot q \vert_{t_1} = 0
\end{equation}

根据哈密顿原理，由于变分$\delta q$和$\delta p$在时间域$(t_0, t_1)$内是任意的，且在端点处的变分也具有独立性，可得出其等价的强形式控制方程及相关的物理边界条件：
\begin{equation}
    \left \{
    \begin{aligned}
        & m\ddot q + k q = 0, \quad & t \in (t_0, t_1) \\
        & m\ddot p + k p = 0, \quad & t \in (t_0, t_1) \\
        & \dot p \vert_{t_0} = \dot q \vert_{t_0}, \\
        & \dot p \vert_{t_1} = \dot q \vert_{t_1}, \\
        & \dot q \vert_{t_1} = 0
    \end{aligned}
    \right .
\end{equation}

根据上述的公式推导，采用了分部积分的方法严格验证了引入拉格朗日乘子的新泛函变分后的弱形式和动力系统原有的强形式控制方程之间的等价性。
这表明，拉格朗日乘子不仅作为辅助数学工具规避了端点限制，
更在物理意义上确立了具备等效完备性的衍生参数空间，
进而从底层保证了时空数值近似离散的可解性和对应性。
...
从上式可以看出，试函数空间和权函数空间均属于相同的离散空间，$\mathcal{Q}_h\times \mathcal{P}_h$。
