...
\begin{figure}[!h]
    \centering
    \includegraphics[scale=0.7]{fig/math/弹簧小车模型.png}
...
f^\alpha_I = \alpha N_I q_0 |_{t=t_0} \\
K^\beta_{IJ} &= \beta N_I N_J |_{t=t_1} \quad
f^\beta_I = \beta N_I \bar p_1 |_{t=t_1}  \\
\end{aligned}
\end{equation}
...
可以得到$2.0$的位移误差收敛率和$1.4$的位移梯度误差收敛率，
均能达到理论的误差收敛率，证明该方案是能够准确收敛的。
...
在拉格朗日乘子时空混合离散计算下，也可以得到如图\ref{fg:tanhuangxiaochefeijunbuL2}所示的$2.0$的位移误差收敛率和$1.6$的位移梯度误差收敛率，
也均能达到理论的误差收敛率。
...
\begin{figure}[!h]
    \centering
    \includegraphics[scale=0.6]{fig/math/简谐弹簧小车模型.png}
...
来测试基于拉格朗日变分方案的哈密顿完全弱形式法的误差收敛率时，
可以得到如图\ref{fg:jxtanhuangxiaocheL2}所示的$2.0$的位移误差收敛率和$1.5$的位移梯度误差收敛率
均能达到理论的误差收敛率，证明该方案是能够准确收敛的。
...
\subsection{双自由度楔形-滑块动力学系统}

本小节算例为一个双自由度无摩擦动力学系统，如图\ref{fg:szyd}。
一个楔形滑块质量为$M=2.0$放置在水平地面上，
另有一个小滑块质量为$m=1.0$放置在该楔形滑块的倾斜面上。
假设所有接触面绝对光滑，不存在摩擦力。其重力加速度为$g=9.8$，
大滑块的倾斜角度为$\theta=30$，初始速度都为零。
在此基础上来计算楔形滑块相对于地面的绝对水平位移和小滑块相对于楔形滑块斜面下滑的相对位移。
\begin{figure}[H]
    \centering
    \includegraphics[scale=1.4]{fig/math/两滑块.png}
\caption{\centering{双自由度楔形-滑块动力学系统模型}}
\label{fg:szyd}
\end{figure}

根据拉格朗日动力学，
有于系统的质量矩阵$\boldsymbol{M}$与外力向量$\boldsymbol{F}$在整个运动过程中均为常数，系统具有恒定的加速度$\boldsymbol{a}$：
\begin{equation}
\boldsymbol{a} = \boldsymbol{M}^{-1} \boldsymbol{F} = \begin{bmatrix} a_1 \\ a_2 \end{bmatrix}
\end{equation}
其中，$a_1$、$a_2$分别为楔形滑块和小滑块的加速度，
结合零初始速度和位移，任意时刻$t$的位移解析解符合标准匀加速运动学公式：
\begin{equation}
\boldsymbol{q}(t) = \frac{1}{2} \boldsymbol{a} t^2 \implies
\begin{cases}
q_2(t) = \frac{1}{2} a_1 t^2 \\
q_1(t) = \frac{1}{2} a_2 t^2
\end{cases}
\end{equation}

通过解析解方程我们可以同样可以采用Newmark-$\beta$法对它进行动力分析，
根据式\eqref{newmark-explicit}-\eqref{newmark-semi-implicit}
所提到的在计算时，根据$\gamma$和$\beta$的取值不同，区分的三种迭代法进行测试。
由于本算例的加速度为常数，因此在采用Newmark-$\beta$法时，它的隐式格式和半隐式格式是一致的。
最后根据Newmark-$\beta$法得到计算结果，
和我们所提出的基于拉格朗日变分方案的哈密顿法的计算结果作对比来测试方法的准确性。

双自由度楔形-滑块动力学系统的求解域由图\ref{fg:bar_szyd}构成，其中总时长$T=2$，
节点个数为$n=4-32$分别对应$\Delta t =0.5-0.0625$的时间步长。

\begin{figure}[H]
    \centering
        \begin{tabular}{c}
            \includegraphics[width=0.8\textwidth]{fig/math/2dn=4.png} \\
            $n=4$ \\
            \includegraphics[width=0.8\textwidth]{fig/math/2dn=8.png} \\
            $n=8$ \\
            \includegraphics[width=0.8\textwidth]{fig/math/2dn=16.png} \\
            $n=16$ \\
            \includegraphics[width=0.8\textwidth]{fig/math/2dn=32.png} \\
            $n=32$
        \end{tabular}
        \caption{双自由度楔形-滑块动力学系统节点均布布置示意图}\label{fg:bar_szyd}
\end{figure}
...
\begin{figure}[H]
    \centering
        \begin{tabular}{c@{\hspace{15pt}}c}
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.5时的位移图.png} &
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.5时的位移误差图.png} \\
        \end{tabular}
        \caption{$\Delta t=0.5$时四种方法计算双自由度楔形-滑块动力学系统的位移图与误差图}\label{fg:szyd0.5}
\end{figure}
\begin{figure}[!h]
    \centering
        \begin{tabular}{c@{\hspace{15pt}}c}
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.25时的位移图.png} &
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.25时的位移误差图.png} \\
        \end{tabular}
        \caption{$\Delta t=0.25$时四种方法计算双自由度楔形-滑块动力学系统的位移图与误差图}\label{fg:szyd0.25}
\end{figure}
\begin{figure}[!h]
    \centering
        \begin{tabular}{c@{\hspace{15pt}}c}
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.125时的位移图.png} &
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.125时的位移误差图.png} \\
        \end{tabular}
        \caption{$\Delta t=0.125$时四种方法计算双自由度楔形-滑块动力学系统的位移图与误差图}\label{fg:szyd0.125}
\end{figure}
\begin{figure}[H]
    \centering
        \begin{tabular}{c@{\hspace{15pt}}c}
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.0625时的位移图.png} &
            \includegraphics[width=0.45\textwidth]{fig/math/双自由度Δt=0.0625时的位移误差图.png} \\
        \end{tabular}
        \caption{$\Delta t=0.0625$时四种方法计算双自由度楔形-滑块动力学系统位移图与误差图}\label{fg:szyd0.0625}
\end{figure}

当我们节点密度采取$4,8,16,32$四种形式，
来测试基于拉格朗日变分方案的哈密顿完全弱形式法的误差收敛率时，双自由度楔形-滑块动力学系统模型
可以得到如图\ref{fg:jxtanhuangxiaocheL2}所示的$2.0$的位移误差收敛率和$1.0$的位移梯度误差收敛率
均能够达到理论的误差收敛率，证明该方案是能够准确收敛的。
 
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.38]{fig/math/双自由度小滑块的L2误差图.png}
\caption{\centering{双自由度楔形-滑块动力学系统的误差收敛结果}}
\label{fg:shuangziyouduL2}
\end{figure}
...
最后，通过对典型的弹簧小车等算例的系统验证，证实了所提方案的优越性。
